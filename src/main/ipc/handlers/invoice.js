...
const printer = await db.all(
  ...
  const companyName = await db.all(
      "SELECT * FROM settings WHERE `key` = ? LIMIT 1",
      ["companyName"]
    );
    const companyPhone = await db.all(
      "SELECT * FROM settings WHERE `key` = ? LIMIT 1",
      ["companyPhone"]
    );
    const companyAddress = await db.all(
      "SELECT * FROM settings WHERE `key` = ? LIMIT 1",
      ["companyAddress"]
    );
...
const data2 = [
 {
        type: 'text',                                      
        value: companyName[0]?.value || 'اسم الشركة',
        style: {fontWeight: "700", textAlign: 'center', fontSize: "24px"}
    },
   {
    type: 'text',
        value: companyPhone[0]?.value || 'رقم هاتف الشركة',
    style: {
        fontWeight: '700',
        textAlign: 'center',
        fontSize: '24px',
    ...
    data2.push(...[
     {
      ...
      {
        type: 'text',                                       // 'text' | 'barCode' | 'qrCode' | 'image' | 'table
          value: companyAddress[0]?.value || 'عنوان الشركة',
        style: {fontWeight: "700", textAlign: 'center', fontSize: "14px"}
    },
    {
    type: 'text',
