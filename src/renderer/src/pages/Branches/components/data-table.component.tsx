...
const DataTable = <T extends Record<string, any>>({
  ...
  const handleSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await dispatch(searchBranches(data as any) as any);
      console.log("search result", result);
      if (!result.error) {
        const formatedBranches = result?.payload?.branches?.map((item: any) => ({
          ...item,
          createdAt: new Date(item.created_at).toISOString().split("T")[0], // Format date to YYYY-MM-DD
        }));
        setTotal(result?.payload?.total || 0);
        setCurrentData(formatedBranches || []);
      } else {
  ...
  const handlePage = async (page: number) => {
    ...
    try {
      const result = await dispatch(getBranches({page}) as any);
      ...
      <div className="overflow-auto rounded-md border">
        <Table className="min-w-full table-auto">
          <TableHeader>
           ...
           {(user.role === "admin" ||
                    user?.permissions?.includes("branches.delete") ||
                    user?.permissions?.includes("branches.edit")) &&   <TableHead className="w-[1%] border-r text-center p-0">
                الإجراءات
              </TableHead>}
            </TableRow>
          </TableHeader>
