...
const CashierPerformanceReport: React.FC<CashierPerformanceReportProps> = ({
  ...
  const activeCashiers = cashiers.filter(c => (c.total_transactions || 0) > 0);

  // Calculate summary statistics
  const totalSales = cashiers.reduce((sum, cashier) => sum + (cashier.total_sales || 0), 0);
  const totalTransactions = cashiers.reduce((sum, cashier) => sum + (cashier.total_transactions || 0), 0);
  ...
  // Find top performer
  const topPerformer = activeCashiers.reduce((top, current) => 
