...
import { monthlySales } from 'store/slices/reportSlice';
...
const MonthlySalesReport: React.FC<MonthlySalesReportProps> = ({
  ...
  const { monthlySales: reportData, loading, error } = useSelector(
  ...
  const dailyChartData = dailyBreakdown.map((day:any) => ({
  ...
  const categoryChartData = categoryPerformance.map((category:any) => ({
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {topProducts.slice(0, 15).map((product:any, index:number) => (
